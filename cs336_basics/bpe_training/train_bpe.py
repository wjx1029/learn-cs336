...
def train_bpe(
    ...
    with open(input_path, "rb") as f:
        ...
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            ...
            代码逻辑：
                ...
                提前加入：训练开始前，强制将特殊 Token 加入词表（通常放在前面），确保它们有 ID。
            """
            if special_tokens:
    ...
    for word_tuple, freq in raw_counts.items():
        words_list.append(list(word_tuple)) # tuple 转换为 list 以便后面修改
    ...
    for idx, word in enumerate(words_list):
        ...
        for i in range(len(word) - 1):
            ...
            pair_freq[pair] += freq          # 累加该 pair 的全局频率
    ...
    for _ in range(num_merges):
        # 如果 `word_freq` 为空（所有可能的对都已合并或频率为0），则停止
        if not pair_freq:
            break
            
        # --- 4a. 寻找最佳 Pair ---
        # 目标：找到当前 `pair_freq` 中频率最高、且字典序最大的 Pair
        # `max(pair_freq.items(), key=lambda x: (x[1], x[0]))` 
            ...
            while i < len(word) - 1:
                ...
                if word[i] == best_pair[0] and word[i+1] == best_pair[1]:
                    ...
                    if i > 0:
                        ...
                        pair_freq[prev_pair] -= freq # 频率减去该单词的频率
                        if pair_freq[prev_pair] == 0:
                            # 如果频率降为 0，从 `pair_freq` 中移除该 pair，避免未来错误选择
                            """
                            pair_freq 字典里依然会存在这个键：{(b'x', b'y'): 0}。
        ...
        # 这个 pair 已经不存在于 `word_freq` 和 `indices` 中了
...
def save_vocab(vocab, output_path):
    ...
    with open(os.path.join(output_path, 'vocab.csv'), 'w', encoding='utf-8') as f:
        for id, token in vocab.items():
            f.write(f"{id},{token.hex()}\n")

    print(f'save vocab.csv to {output_path}.')


def save_merges(merges, output_path):
    os.makedirs(output_path, exist_ok=True)

    with open(os.path.join(output_path, 'merges.csv'), 'w', encoding='utf-8') as f:
        for pair in merges:
            f.write(f"{pair[0].hex()},{pair[1].hex()}\n")

    print(f'save merges.csv to {output_path}.')
